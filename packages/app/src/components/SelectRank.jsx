...
const Option = ({ option, selected, select, handleTab, close }) => {
  ...
  return (
    <li
      className={
        hovered
          ? 'text-slate-300 bg-slate-700 cursor-pointer select-none relative py-2 pl-3 pr-9'
          : 'text-slate-300 bg-slate-800 cursor-pointer select-none relative py-2 pl-3 pr-9'
      }
      id={`role-option-${option.name}`}
      tabIndex="0"
      onKeyDown={(e) => handleTab(e)}
    ...
    <div>
      ...
      <div className="mt-1 relative" ref={ref}>
        <button
          ...
          type="button"
          className="relative w-full min-w-fit h-14 bg-slate-800 border border-slate-800 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-pointer focus:outline-none focus:ring-1 focus:ring-slate-500 focus:border-slate-500 sm:text-sm"
          aria-haspopup="listbox"
          aria-expanded="true"
          aria-labelledby={`player-${i + 1}-elo`}
        >
