...
module.exports = function (games) {
  let options = games
  const laneDeltaThreshold = quantile(
    options.map((x) => x.laneDelta),
    0.5
  )
  options = filterLaneDelta(options, laneDeltaThreshold)
  const roleScoreThreshold = quantile(
    options.map((x) => x.roleScore),
    0.5
  )
  options = filterRoleScore(options, roleScoreThreshold)
  const deltaValuesThreshold = quantile(
    options.map((x) => x.delta),
    0.5
  )
  options = filterDelta(options, deltaValuesThreshold)
  const skillLevelThreshold = quantile(
    options.map((x) => x.skillLevel),
    0.5
  )
  ...
  return options.sort((a, b) => {
    if (a.delta - b.delta === 0) {
      if (a.laneDelta - b.laneDelta === 0) {
        if (b.roleScore - a.roleScore === 0) return b.skillLevel - a.skillLevel
        else return b.roleScore - a.roleScore
      } else return a.laneDelta - b.laneDelta
    }
