...
const autofillProtection = (lobbies, players) => {
  const autofillCount = lobbies.map(
    (x) =>
      x.filter(
        (y, i) =>
          !(
            players[y].roles.length === 0 ||
            players[y].roles.includes(ROLES_BY_INDEX[i])
          )
      ).length
  )
...
module.exports = function (players, autofill) {
  ...
  if (autofill) return lobbies
  else return autofillProtection(lobbies, players)
}
