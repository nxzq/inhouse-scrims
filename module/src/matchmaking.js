const { MMR, MMR_MODIFIER, ROLES_BY_INDEX } = require('./utils/enums')
...
const getRoleStatusArray = (players) => {
  return players.map((player, i) => {
    ...
    let status = 'autofill'
    if (player.roles.length === 0) status = 'fill'
    if (player.roles[0] === role) status = 'primary'
    if (player.roles[1] && player.roles[1] === role) status = 'secondary'
    return status
  })
}
...
const getDraftMetadata = (draft, players) => {
  ...
  const mmrBreakdown = lobby.map(
    (x, i) => MMR[x.elo] - MMR_MODIFIER[rolesBreakdown[i]]
  )
...
const prettyOutput = (game, players) => {
  ...
  const team1 = {
    roster: {
      top: {
        ...roster[0],
        autofill: rolesBreakdown[0] === 'autofill' ? true : false,
        mmr: mmrBreakdown[0],
        elo: prettyRank(roster[0].elo),
      },
      jug: {
        ...roster[1],
        autofill: rolesBreakdown[1] === 'autofill' ? true : false,
        mmr: mmrBreakdown[1],
        elo: prettyRank(roster[1].elo),
      },
      mid: {
        ...roster[2],
        autofill: rolesBreakdown[2] === 'autofill' ? true : false,
        mmr: mmrBreakdown[2],
        elo: prettyRank(roster[2].elo),
      },
      bot: {
        ...roster[3],
        autofill: rolesBreakdown[3] === 'autofill' ? true : false,
        mmr: mmrBreakdown[3],
        elo: prettyRank(roster[3].elo),
      },
      sup: {
        ...roster[4],
        autofill: rolesBreakdown[4] === 'autofill' ? true : false,
        mmr: mmrBreakdown[4],
        elo: prettyRank(roster[4].elo),
      },
    },
    mmr: team1Mmr,
  }
  const team2 = {
    roster: {
      top: {
        ...roster[5],
        autofill: rolesBreakdown[5] === 'autofill' ? true : false,
        mmr: mmrBreakdown[5],
        elo: prettyRank(roster[5].elo),
      },
      jug: {
        ...roster[6],
        autofill: rolesBreakdown[6] === 'autofill' ? true : false,
        mmr: mmrBreakdown[6],
        elo: prettyRank(roster[6].elo),
      },
      mid: {
        ...roster[7],
        autofill: rolesBreakdown[7] === 'autofill' ? true : false,
        mmr: mmrBreakdown[7],
        elo: prettyRank(roster[7].elo),
      },
      bot: {
        ...roster[8],
        autofill: rolesBreakdown[8] === 'autofill' ? true : false,
        mmr: mmrBreakdown[8],
        elo: prettyRank(roster[8].elo),
      },
      sup: {
        ...roster[9],
        autofill: rolesBreakdown[9] === 'autofill' ? true : false,
        mmr: mmrBreakdown[9],
        elo: prettyRank(roster[9].elo),
      },
    },
    mmr: team2Mmr,
  }
...
module.exports = function (data) {
  const players = data.map((x) => {
    return { ...x, mmr: MMR[x.elo] }
  })
  const roleDiversity = [
    ...new Set(players.map((x) => JSON.stringify(x.roles))),
  ]
