...
export default function SummonerInput({ summoner, i, handleChange }) {
  return (
    <div style={{ padding: '.5em', margin: '.5em', border: '1px grey solid' }} >
      <p style={{ color: 'grey' }}>Player {i}</p>
      <div style={{ display: 'flex', justifyContent: 'flex-start', flexWrap: 'wrap' }}>
        <div style={{ marginRight: '.25em'}}>
          <label>
            Name:&nbsp;
            <input type="text" value={summoner.name} name="name" onChange={(e) => handleChange(e,i)} style={{ marginRight: '.25em'}} />
          </label>
        </div>
        <label style={{ marginRight: '.25em'}}>
          Elo:&nbsp;
          <select name="elo" value={summoner.elo} onChange={(e) => handleChange(e,i)} style={{ marginRight: '.25em'}}>
        ...
        <label style={{ marginRight: '.25em'}}>
          Roles:&nbsp;
          <select name="roles" value={summoner.roles} onChange={(e) => handleChange(e,i)} style={{ marginRight: '.25em'}}>
